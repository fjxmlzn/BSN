...
class GAN(object):
    ...
    def build_connection(self):
        ...
        self.d_real_image_train_tf, _, self.d_sigmas, self.d_scales = \
    ...
    def build_loss(self):
        ...
        # discriminator
        self.d_loss_fake = tf.reduce_mean(self.d_fake_image_train_tf)
        self.d_loss_real = -tf.reduce_mean(self.d_real_image_train_tf)

        alpha = tf.random_uniform(
            shape=[self.batch_size, 1, 1, 1],
            minval=0.,
            maxval=1.)
        differences = self.fake_image_train_tf - self.real_image_pl
        interpolates = self.real_image_pl + alpha * differences
        ...
        slopes = tf.sqrt(slopes + self.EPS)
        self.d_loss_gp = (slopes - 1.)**2
        self.d_loss_gp = tf.reduce_mean(self.d_loss_gp)
    ...
    def build_summary(self):
        ...
        self.d_summary.append(tf.summary.scalar(
            "loss/d/gp", tf.reduce_mean(self.d_loss_gp)))
        self.d_summary.append(tf.summary.scalar(
            "loss/d", self.d_loss))
        self.d_summary.append(tf.summary.scalar(
            "d/real",
        ...
        for i in range(len(self.d_scales)):
            self.d_summary.append(tf.summary.scalar(
                "d/scale{}".format(i),
                self.d_scales[i]))
